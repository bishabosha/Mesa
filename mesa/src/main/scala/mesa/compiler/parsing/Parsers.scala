...
private[parsing] val mesaParser =
...
def [C](parser: String => C) toTreeParser(toTree: IdReader[C => Lifted[Tree]])(input: String)(using IdGen): Lifted[Tree] = {
  import CompilerErrorOps._
  for
...
private def genParser(input: String): MesaParser = {
  ...
  lexer.addErrorListener(mesaErrorListener)
  parser.removeErrorListeners
  parser.addErrorListener(mesaErrorListener)
  parser
}
