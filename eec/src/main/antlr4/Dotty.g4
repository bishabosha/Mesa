...
typeParamBounds
    :   (':' type)*                    //    ContextBounds(typeBounds, tps)
    ;
...
simpleExpr1
    ...
    |  simpleExpr1 (typeArgs | namedTypeArgs)  //                  TypeApply(expr, args)
...
clsParam
    :   (modifier* 'val')? param      //              ValDef(mods, id, tpe, expr) -- point of mods on val/var
    ;
...
binding
    :  (Id | '_') (':' type)?                   //                 ValDef(_, id, tpe, EmptyTree)
    ;
...
importExpr
    :   stableId '.' (Id | '_' | importSelectors)         //       Import(expr, sels)
    ;
...
typeDcl
    :   Id typTypeParamClause? ('=' type)        //     TypeDefTree(_, name, tparams, bounds)
    ;
...
classConstr
    :   clsTypeParamClause? clsParamClauses      //  with DefDef(_, <init>, Nil, vparamss, EmptyTree, EmptyTree) as first stat
    ;
