...
object Printing {
  ...
  object untyped {
    ...
    enum Ast {
      ...
      case DefDef(modifiers: Set[Modifier], sig: Ast, typ: Ast, value: Ast)
    ...
    object AstOps {
      ...
      def (tree: Tree) toAst: Ast = tree match {
        ...
        case Tree.DefDef(_, modifiers, sig, typ, value) => DefDef(modifiers, sig.toAst, typ.toAst, value.toAst)
